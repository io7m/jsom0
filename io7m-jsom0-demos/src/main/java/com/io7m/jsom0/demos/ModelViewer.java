...
public final class ModelViewer
{
  ...
  {
    final InputStream stream = new FileInputStream(file);
    final ModelParser parser = ModelParser.makeParser(file, stream, gl, log);
    final Model m = parser.model();
    stream.close();
    return m;
  }

  private static ModelObject loadObject(
    final String object_name,
    final Model m,
    final Log log)
    throws ConstraintError
  {
    final Option<ModelObject> oo = m.get(object_name);
    if (oo.type == Type.OPTION_NONE) {
      final StringBuilder builder = new StringBuilder();

      log.error("could not find object '" + object_name + "' in model");
      m.forEachObject(new Function<ModelObject, Unit>() {
        @Override public Unit call(
          final @Nonnull ModelObject x)
        {
    ...
    final Option.Some<ModelObject> so = (Some<ModelObject>) oo;
    return so.value;
  }
  ...
  private final @Nonnull ModelObject         object;
      ...
      ConstraintError
  {
    ...
    this.program.activate(g);
    {
      ...
      switch (this.material.texture.type) {
        case OPTION_SOME:
        {
          g.texture2DRGBAStaticBind(this.texture_units[0], this.texture);
